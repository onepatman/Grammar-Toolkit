...
describe("the owner's Language Bank additions reach the shared Firestore doc", () => {
  it("seeds an empty languageBank object (all 6 categories) alongside entries on first connect", async () => {
    ...
    expect(doc.languageBank).toEqual({ phrasal: [], idioms: [], sentences: [], patterns: [], technical: [], phraseTemplates: [] });
  });
...
describe("a device that connects to an already-seeded code pulls in the shared Language Bank entries", () => {
  ...
  it("pulls in entries across all 6 categories at once", async () => {
    const firebase = makeFirebase();
    firebase._docs.set("syncedLogs/lb-code-6", {
      ...
      languageBank: {
        ...
        phraseTemplates: [{ w: "Email: Confirming receipt of a message", senses: [{ use: "u", examples: [] }], syn: [], ant: [], mistake: null, tagalog: null, source: "online" }]
      }
    });
    ...
    expect(hooks.phraseTemplatesData.some((p) => p.w === "Email: Confirming receipt of a message")).toBe(true);
  });
